...
export class SceneRenderer {
    constructor(context) {
        ...
        this.embers = [];
        this.gusts = [];
        this.mountainFar = null;
        this.mountainNear = null;
    ...
    updateAmbient(time, delta, state) {
        for (const cloud of this.clouds) {
            cloud.x += cloud.velocity * delta / 1000;
            if (cloud.x > this.width + 320)
                cloud.x = -320;
            if (cloud.x < -320)
                cloud.x = this.width + 320;
        }
        for (const ember of this.embers) {
            ...
            if (ember.y < -10) {
                ...
                ember.x = randomBetween(0, this.width);
            }
        }
        ...
        for (const gust of this.gusts)
            gust.x += gust.velocity * delta / 1000;
        this.gusts = this.gusts.filter(gust => gust.x < this.width + 320);
    }
    render(time, state, player, enemy) {
        ...
        this.drawClouds();
        if (this.mountainFar)
            ctx.drawImage(this.mountainFar, 0, 0);
        if (this.mountainNear)
            ctx.drawImage(this.mountainNear, 0, 0);
        this.drawGround();
        this.drawGrass(time);
    ...
    regenerateBackground() {
        ...
        this.cloudSprite ?? (this.cloudSprite = this.makeCloudSprite());
    }
    makeMountain(base, amplitude, color) {
    ...
    makeCloudSprite() {
        ...
        const g = canvas.getContext('2d');
        if (!g)
            throw new Error('Cloud canvas context unavailable');
        const blob = (x, y, radius) => {
    ...
    drawSky() {
        ...
        g.addColorStop(.45, '#1b2148');
        g.addColorStop(.75, '#2a2b57');
        g.addColorStop(1, '#463a5c');
    ...
    drawStars(time) {
        for (const star of this.stars) {
            ...
            this.context.arc(star.x, star.y, star.radius, 0, TAU);
            this.context.fill();
        }
    }
    drawMoon() {
        ...
        g.addColorStop(0, 'rgba(250,232,170,.30)');
        g.addColorStop(1, 'rgba(250,232,170,0)');
        ctx.fillStyle = g;
        ...
        ctx.fill();
        ctx.fillStyle = 'rgba(190,160,90,.18)';
        ctx.beginPath();
        ctx.arc(mx - radius * .3, my - radius * .2, radius * .18, 0, TAU);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(mx + radius * .25, my + radius * .3, radius * .12, 0, TAU);
        ctx.fill();
    }
    drawClouds() {
        if (!this.cloudSprite)
    ...
    drawGround() {
        ...
        ctx.strokeStyle = 'rgba(240,194,94,.22)';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        g.addColorStop(0, 'rgba(60,70,130,.14)');
    ...
    drawGrass(time) {
        ...
        for (const grass of this.grass) {
            ...
            ctx.strokeStyle = 'rgba(20,26,52,.95)';
            ctx.lineWidth = grass.width;
            ctx.beginPath();
            ctx.moveTo(grass.x, this.height + 4);
            ctx.quadraticCurveTo(grass.x + sway * .3, this.height - grass.height * .55, grass.x + sway, this.height - grass.height);
            ctx.stroke();
            ctx.fillStyle = grass.tip ? 'rgba(240,194,94,.5)' : 'rgba(120,130,180,.4)';
            ctx.beginPath();
            ctx.ellipse(grass.x + sway, this.height - grass.height, 2.4, 5.5, sway * .05, 0, TAU);
            ctx.fill();
        }
    }
    drawEmbersAndGusts() {
        const ctx = this.context;
        for (const ember of this.embers) {
            ctx.fillStyle = `rgba(${ember.color},.55)`;
            ctx.beginPath();
        ...
        for (const gust of this.gusts) {
            ...
            g.addColorStop(.5, `rgba(220,230,255,${gust.alpha})`);
            g.addColorStop(1, 'rgba(220,230,255,0)');
            ctx.fillStyle = g;
            ctx.fillRect(gust.x, gust.y, gust.length, 1.6);
        }
    }
    shadow(x, width) {
        this.context.fillStyle = 'rgba(0,0,0,.38)';
        this.context.beginPath();
        this.context.ellipse(x, this.groundY + 4, width, width * .22, 0, 0, TAU);
        this.context.fill();
    }
    drawFighter(fighter, time, state, enemy) {
        const ctx = this.context, r = fighter.radius, elapsed = time - fighter.t0, palette = fighter.palette;
        let rotation = 0, squash = 1, sword = 2.6, face = 'idle', bob = 0, noHat = false, noSword = false;
        switch (fighter.pose) {
            case 'idle':
                bob = Math.sin(time / 620 + fighter.seed) * r * .04;
                sword = 2.6 + Math.sin(time / 900 + fighter.seed) * .06;
                face = Math.sin(time / 900 + fighter.seed) > .985 ? 'blink' : 'idle';
                break;
            case 'enter':
                bob = -Math.abs(Math.sin(elapsed / 130)) * 5 * this.scale;
                break;
            case 'stance': {
                const k = Math.min(1, elapsed / 200);
                squash = 1 - .09 * k;
                rotation = .06 * k * fighter.facing;
                sword = -2.7;
                face = 'sharp';
                break;
            }
            case 'lunge':
            case 'slashWin': {
                const duration = fighter.pose === 'lunge' ? fighter.lungeDuration : 140, k = Math.min(1, elapsed / duration);
                rotation = .14 * k * fighter.facing;
                squash = .96;
                sword = -2.7 + 3.1 * Math.min(1, elapsed / (duration * .8));
                face = 'sharp';
                break;
            }
            case 'whiff': {
                const k = Math.min(1, elapsed / 220);
                sword = 2.6 - 3 * Math.min(1, k * 1.7);
                rotation = .08 * fighter.facing * Math.sin(k * Math.PI);
                break;
            }
            case 'stand2':
                sword = .6;
                face = 'sharp';
                rotation = .04 * fighter.facing;
                break;
            case 'fall':
                rotation = fighter.rot;
                face = 'dizzy';
                noHat = true;
                noSword = true;
                break;
        ...
        const drawSword = () => {
            ...
            ctx.beginPath();
            ctx.moveTo(sx - Math.cos(sword) * r * .4, sy - Math.sin(sword) * r * .4);
            ctx.lineTo(sx, sy);
            ctx.stroke();
            const gradient = ctx.createLinearGradient(sx, sy, ex, ey);
            gradient.addColorStop(0, '#cfd6ea');
            ...
            ctx.beginPath();
            ctx.arc(sx, sy, 3.6 * this.scale, 0, TAU);
            ctx.fill();
        };
        if (!noSword && Math.cos(sword) < 0)
            drawSword();
        ...
        ctx.save();
        ctx.beginPath();
        ctx.arc(0, cy, r - 1.5 * this.scale, 0, TAU);
        ctx.clip();
        ctx.fillStyle = palette.scarf;
        ctx.fillRect(-r, cy + r * .28, r * 2, r * .34);
        const flap = Math.sin(time / 150 + fighter.seed) * 4 * this.scale;
        ctx.beginPath();
        ctx.moveTo(-r * .9, cy + r * .4);
        ctx.lineTo(-r * 1.5, cy + r * .3 + flap);
        ctx.lineTo(-r * .95, cy + r * .62);
        ctx.closePath();
        ctx.fill();
        ctx.restore();
        const eyeColor = fighter.glow ? '#ff5a44' : palette.eye;
        if (fighter.glow) {
            ctx.shadowColor = 'rgba(255,80,60,.9)';
        ...
        if (face === 'dizzy') {
            ...
            for (const ex of [r * .16, r * .5]) {
                ctx.beginPath();
                ctx.moveTo(ex - 4 * this.scale, eyeY - 4 * this.scale);
                ctx.lineTo(ex + 4 * this.scale, eyeY + 4 * this.scale);
                ctx.moveTo(ex + 4 * this.scale, eyeY - 4 * this.scale);
                ctx.lineTo(ex - 4 * this.scale, eyeY + 4 * this.scale);
                ctx.stroke();
            }
            ctx.beginPath();
            ctx.arc(r * .33, cy - r * .02, 4 * this.scale, 0, TAU);
            ctx.fill();
        }
        else {
            ...
            for (const ex of [r * .16, r * .5]) {
                ctx.beginPath();
                ctx.ellipse(ex, eyeY, r * .06, Math.max(1.5 * this.scale, ry), 0, 0, TAU);
                ctx.fill();
            }
            if (face === 'sharp') {
                ctx.lineWidth = 2.4 * this.scale;
                ctx.beginPath();
                ctx.moveTo(r * .06, cy - r * .5);
                ctx.lineTo(r * .3, cy - r * .44);
                ctx.moveTo(r * .64, cy - r * .5);
                ctx.lineTo(r * .42, cy - r * .44);
                ctx.stroke();
            }
            ctx.lineWidth = 2.4 * this.scale;
            ctx.beginPath();
            if (face === 'idle')
                ctx.arc(r * .34, cy - r * .05, 4 * this.scale, .2, Math.PI - .4);
            else {
                ctx.moveTo(r * .24, cy - r * .04);
                ctx.lineTo(r * .46, cy - r * .04);
            }
            ctx.stroke();
        }
        ctx.shadowBlur = 0;
        if (!noSword && Math.cos(sword) >= 0)
            drawSword();
        if (!noHat) {
            ctx.fillStyle = palette.hatD;
            ctx.strokeStyle = palette.line;
            ctx.lineWidth = 3 * this.scale;
            ctx.beginPath();
            ctx.ellipse(0, cy - r * .82, r * 1.08, r * .3, 0, 0, TAU);
            ctx.fill();
            ctx.stroke();
            ctx.fillStyle = palette.hat;
            ctx.beginPath();
            ctx.moveTo(0, cy - r * 1.62);
            ctx.quadraticCurveTo(-r * .7, cy - r * 1.1, -r * 1.05, cy - r * .84);
            ctx.quadraticCurveTo(0, cy - r * .66, r * 1.05, cy - r * .84);
            ctx.quadraticCurveTo(r * .7, cy - r * 1.1, 0, cy - r * 1.62);
            ctx.closePath();
            ctx.fill();
            ctx.stroke();
            ctx.strokeStyle = palette.scarf;
            ctx.lineWidth = 3 * this.scale;
            ctx.beginPath();
            ctx.moveTo(-r * .62, cy - r * 1.05);
            ctx.quadraticCurveTo(0, cy - r * .9, r * .62, cy - r * 1.05);
            ctx.stroke();
        }
        ctx.restore();
        if (fighter === enemy && ['intro', 'wait', 'flash'].includes(state.state)) {
            ctx.font = `${13 * this.scale + 8}px sans-serif`;
            ctx.textAlign = 'center';
            ctx.fillStyle = 'rgba(240,194,94,.85)';
            ctx.fillText(fighter.meta?.name ?? '', fighter.x, this.groundY - fighter.radius * 2.35 + fighter.fy);
        }
    }
    drawHats(state) {
        const ctx = this.context;
        for (const hat of state.hats) {
            const alpha = clamp(1 - (hat.age - .8) / .6, 0, 1);
            ctx.save();
            ctx.globalAlpha = alpha;
            ctx.translate(hat.x, hat.y);
            ctx.rotate(hat.rot);
            ctx.fillStyle = hat.palette.hat;
            ctx.strokeStyle = hat.palette.line;
            ctx.lineWidth = 2 * this.scale;
            ctx.beginPath();
            ctx.moveTo(0, -hat.radius * .5);
            ctx.lineTo(-hat.radius * .8, hat.radius * .1);
            ctx.quadraticCurveTo(0, hat.radius * .28, hat.radius * .8, hat.radius * .1);
            ctx.closePath();
            ctx.fill();
            ctx.stroke();
            ctx.restore();
    ...
    drawBang(time, state) {
        ...
        let alpha = 1;
        if (state.state === 'win')
            alpha = Math.max(0, 1 - age / 260);
        if (state.state === 'lose' && state.lose)
            alpha = Math.max(0, 1 - (time - state.lose.t0) / 420);
        if (alpha <= 0) {
            state.bangOn = false;
            return;
        }
        ...
        ctx.lineWidth = 3 * this.scale;
        for (let i = 0; i < 8; i++) {
            const angle = i / 8 * TAU + age * .002;
            ctx.beginPath();
            ctx.moveTo(cx + Math.cos(angle) * size * .72, cy + Math.sin(angle) * size * .72);
        ...
        ctx.shadowColor = 'rgba(255,70,40,.9)';
        ctx.shadowBlur = 30;
    ...
    drawFx(time, state) {
        ...
        for (const particle of state.particles) {
            ...
            ctx.fillStyle = `rgba(${particle.color},${particle.soft ? alpha * .45 : alpha})`;
            ctx.beginPath();
        ...
        if (state.trail) {
            ...
            if (age < 280) {
                const alpha = 1 - age / 280, trail = state.trail;
                ctx.strokeStyle = `rgba(255,255,245,${alpha * .95})`;
                ctx.lineWidth = 9 * this.scale * alpha + 1;
                ctx.lineCap = 'round';
                ctx.beginPath();
                ctx.moveTo(trail.x - 250 * this.scale, trail.y - 160 * this.scale);
                ctx.lineTo(trail.x + 250 * this.scale, trail.y + 150 * this.scale);
                ctx.stroke();
                ctx.strokeStyle = `rgba(240,194,94,${alpha * .6})`;
        ...
        for (const floating of state.floats) {
            const alpha = 1 - floating.age / .95;
            ctx.font = `${(floating.big ? 24 : 15) * this.scale}px sans-serif`;
            ctx.textAlign = 'center';
            ctx.fillStyle = floating.color;
            ctx.globalAlpha = alpha;
            ctx.fillText(floating.text, floating.x, floating.y - floating.age * 60 * this.scale);
            ctx.globalAlpha = 1;
        }
    }
}
